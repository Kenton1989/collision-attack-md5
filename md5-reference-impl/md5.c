...
void md5Step(uint32_t *buffer, uint32_t *input) {
    ...
    unsigned int j;
    printf("step,input A,B,C,D,\"f(B,C,D)\",msg idx,msg,output,before rot,after rot\n");
    for (unsigned int i = 0; i < 64; ++i) {
        ...
        uint32_t before_rot = AA + E + K[i] + input[j];
        uint32_t after_rot = rotateLeft(before_rot, S[i]);

        printf("%02d,0x%08x,0x%08x,0x%08x,0x%08x,0x%08x,%02d,0x%08x,0x%08x,0x%08x,0x%08x\n",
               i, AA, BB, CC, DD, E, j, input[j], BB + after_rot, before_rot, after_rot);
